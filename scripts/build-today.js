...
// 페르소나 시각 — 직책별 해석 패턴 (Round 6: 실명 → 직책명).
function personaInterpret(expert, { topNews, secondNews, topOss, topCom }) {
  const role = expert.role || "";
  // Round 5의 ko 우선 정책 — 한글 번역이 있으면 그것을 사용
  const newsTitle = truncate((topNews.title_ko || topNews.title), 60);
  const intro = `**${expert.name}** (${role}) 시각에서 이 신호는`;

  const interpretations = {
    "chairman":         `${intro} 5-10년 시간선의 사업 포트폴리오 시그널이다. "${newsTitle}"가 단순 트렌드가 아닌 산업 구조 변화의 초기 신호인지를 판별해야 한다. 경쟁사가 같은 방향으로 움직이고 있는지, 우리가 선도/추격/회피 중 어느 입장이어야 하는지를 결정해야 할 시점.`,
    "dx-head":          `${intro} DX 부문의 1억+ 고객 단말 경험에 직접 영향을 줄 수 있는 시그널이다. "${newsTitle}"가 갤럭시·TV·가전 통합 사용자 경험의 다음 차별화 포인트가 될 수 있는지, 통합 시점은 언제인지를 판단해야 한다.`,
    "ds-head":          `${intro} HBM·파운드리·시스템 LSI 의 R&D 우선순위에 영향을 주는 시그널이다. "${newsTitle}"가 SK하이닉스·TSMC·인텔과의 격차 구도를 어떻게 바꾸는지, 우리 capa·wafer 할당을 재조정해야 하는지를 판단해야 한다.`,
    "mx-head":          `${intro} 갤럭시 다음 세대 단말의 차별화 자산이 될 수 있는 시그널이다. "${newsTitle}"가 iPhone과의 격차를 좁히거나 폴더블 우위를 확장할 수 있는 카드인지, 온디바이스 vs 클라우드 분배는 어떻게 해야 하는지를 결정해야 한다.`,
    "cmo":              `${intro} 200개국 글로벌 마케팅 메시지의 새 축이 될 수 있는 시그널이다. "${newsTitle}"를 어떤 스토리로 풀어 Apple/Google과 차별화할지, Galaxy Unpacked 메시지 전략에 어떻게 통합할지를 판단해야 한다.`,
    "da-head":          `${intro} DA(생활가전) marginal cost와 SmartThings 생태계에 영향을 주는 시그널이다. "${newsTitle}"가 LG·Haier·Midea와의 가전 경쟁에서 우리의 IoT/AI 우위를 강화할지, 회수 시간선은 얼마인지를 판단해야 한다.`,
    "foundry-head":     `${intro} 파운드리 고객 확보 경쟁에서의 시그널이다. "${newsTitle}"가 NVIDIA/Qualcomm/Tesla 같은 핵심 고객의 의사결정에 영향을 주는지, TSMC와의 yield·node 격차를 좁힐 기회를 만드는지를 판단해야 한다.`,
    "lsi-head":         `${intro} Exynos·ISOCELL 로드맵에 직접 영향을 주는 시그널이다. "${newsTitle}"가 Snapdragon과의 SoC 경쟁에서 차세대 우위를 만들 수 있는지, 이미지 센서 시장 점유율 확대에 활용할 수 있는지를 판단해야 한다.`,
    "vd-head":          `${intro} TV/모니터 ASP와 콘텐츠 생태계에 영향을 주는 시그널이다. "${newsTitle}"가 Sony·LG의 OLED 공세를 방어할 카드인지, TCL·Hisense의 가성비 공세에 대응할 무기가 될 수 있는지를 판단해야 한다.`,
    "memory-head":      `${intro} HBM·DDR·LPDDR 수요 곡선의 변곡점 시그널이다. "${newsTitle}"가 NVIDIA H200/B100 인증 또는 차세대 AI 서버 메모리 표준에 어떤 영향을 주는지, HBM4·CXL 양산 시점에 시사하는 것이 무엇인지를 판단해야 한다.`,
    "networks-head":    `${intro} 5G/6G 장비 수주 환경에 영향을 주는 시그널이다. "${newsTitle}"가 Verizon·AT&T·NTT 고객 결정에 어떤 영향을 주는지, Open RAN/vRAN 채택 추세에 우리가 어떻게 대응해야 하는지를 판단해야 한다.`,
    "ds-cto":           `${intro} 10년 시간선 R&D 로드맵에 직접 영향을 주는 시그널이다. "${newsTitle}"가 EUV·GAA·차세대 패키징 기술의 발전 속도를 어떻게 바꾸는지, 우리 차세대 노드 결정에 어떤 시사점이 있는지를 판단해야 한다.`,
    ...
    "biz-support-tf":   `${intro} 사업부 간 자원 재분배·M&A·인사 정책에 영향을 주는 시그널이다. "${newsTitle}"가 우리 조직 구조나 우선순위를 어떻게 재조정하게 만드는지, 다른 사업부에 미치는 파급은 어떤지를 판단해야 한다.`,
    "cfo":              `${intro} CapEx 결정과 IRR/NPV 시간선에 영향을 주는 시그널이다. "${newsTitle}"가 우리 향후 3년 투자 우선순위를 어떻게 바꾸는지, ROI 회수 시점이 어떻게 변동하는지를 정량적으로 판단해야 한다.`,
    "public-affairs":   `${intro} 미국 CHIPS Act, EU AI Act, 한국 K-반도체 정책 등 글로벌 규제 환경의 시그널이다. "${newsTitle}"가 우리 미국·EU·중국 사업의 제약·기회를 어떻게 바꾸는지, 정부 인센티브 협상에서 어떻게 활용할 수 있는지를 판단해야 한다.`,
    "ma-head":          `${intro} 검토 중인 M&A pipeline의 가치·우선순위에 영향을 주는 시그널이다. "${newsTitle}"가 인수 후보의 valuation을 어떻게 바꾸는지, antitrust 심사 환경은 어떤지, 대안 후보가 등장했는지를 판단해야 한다.`,
    "us-ceo":           `${intro} 미국 시장 channel·통신사 협력·정부 입찰에 영향을 주는 시그널이다. "${newsTitle}"가 Verizon·AT&T·Best Buy와의 관계, FCC 규제, Texas Fab 운영에 어떤 영향을 주는지를 판단해야 한다.`,
    ...
    "strategy":         `${intro} 2-3년 후 시장 구조에 대한 시그널이다. "${newsTitle}"가 Apple/Google/Meta/TSMC/SK하이닉스/LG의 경쟁 구도를 어떻게 재편하는지, 우리가 지금 준비해야 할 카드는 무엇인지를 판단해야 한다.`,
  };
...
function personaRecommend(expert, { topNews, topOss, topCom }) {
  const recommendations = {
    "chairman":         `1. 이 트렌드와 관련된 글로벌 경쟁사(Apple/TSMC/NVIDIA/SK하이닉스)의 동향을 향후 4주간 모니터.\n2. 우리 사업 포트폴리오 중 ${topNews.category || "관련"} 카테고리에 추가 투자할지를 다음 분기 회의에서 결정.`,
    "dx-head":          `1. DX 사업부장 회의에서 이 기술의 갤럭시·TV·가전 통합 우선순위를 정렬.\n2. 6개월 내 PoC 가능 여부를 R&D 팀에 요청하고, Galaxy Unpacked 일정과 동기화.`,
    "ds-head":          `1. 이 시그널이 HBM·DDR·파운드리 capa 수요 예측에 미치는 영향을 12주 내 정량화.\n2. SK하이닉스·TSMC·인텔의 동일 트렌드 대응 모니터링 강화.`,
    "mx-head":          `1. 다음 갤럭시 라인업의 차별화 핵심으로 통합 가능한지 MX UX 팀과 검토.\n2. 온디바이스 vs 클라우드 비중 결정을 위한 latency/cost 벤치마크 의뢰.`,
    "cmo":              `1. 200개국 마케팅 메시지에 이 트렌드를 어떻게 통합할지 글로벌 캠페인 팀과 5주 내 결정.\n2. Apple 마케팅 동향과 비교한 차별화 포인트 도출.`,
    "da-head":          `1. SmartThings 생태계 통합 우선순위를 6주 내 재검토.\n2. LG·Haier·Midea의 동일 트렌드 대응 동향 추적.`,
    "foundry-head":     `1. NVIDIA·Qualcomm·Tesla 등 핵심 파운드리 고객의 결정 동향을 4주 내 점검.\n2. 2nm/3nm 양산 일정에 영향이 있다면 우선순위 재조정.`,
    "lsi-head":         `1. Exynos 차세대 로드맵에 통합 가능 여부를 SoC 팀과 8주 내 검토.\n2. ISOCELL 이미지 센서 차세대 모델 개발에 시사점이 있는지 확인.`,
    "vd-head":          `1. Neo QLED·OLED·micro-LED 라인업 가격 정책에 영향이 있는지 검토.\n2. Tizen OS 광고 플랫폼에 통합 가능 여부를 4주 내 결정.`,
    "memory-head":      `1. HBM4·CXL·LPDDR6 양산 일정 검토 — 이 시그널이 NVIDIA 인증 일정에 영향이 있는지.\n2. 메모리 마진 회복 시점 예측 업데이트.`,
    "networks-head":    `1. Verizon·AT&T·NTT 고객 미팅에서 이 시그널이 5G/6G 의사결정에 영향이 있는지 청취.\n2. Open RAN 표준화 동향 모니터링 강화.`,
    "ds-cto":           `1. 차세대 R&D 로드맵에 이 기술 변화를 반영할 시점을 8주 내 결정.\n2. 학계·오픈소스 동향에서 추가 신호가 있는지 분기 별 점검.`,
    "dx-cto":           `1. 갤럭시·TV·가전 중 우선 통합 단말을 4주 내 결정.\n2. 외부 파트너십·인수 옵션이 있는지 Samsung NEXT와 검토.`,
    "biz-support-tf":   `1. 이 시그널이 사업부 간 자원 재분배에 영향이 있는지 부회장단 회의에서 검토.\n2. 인사 정책 또는 조직 구조 변경이 필요한지 8주 내 판단.`,
    "cfo":              `1. 향후 3년 CapEx 우선순위에 미치는 영향을 IR 팀과 정량화.\n2. 환율·금리 시나리오에 따른 ROI 회수 시점 재계산.`,
    "public-affairs":   `1. 미국·EU·중국 정부 관계자 미팅에서 이 정책 변화의 의미를 4주 내 청취.\n2. CHIPS Act/AI Act 인센티브 협상에 활용 가능한지 검토.`,
    "ma-head":          `1. M&A pipeline 후보의 valuation 재산정.\n2. 경쟁 인수자의 동향 모니터링 강화.`,
    "us-ceo":           `1. 미국 통신사·유통 채널 미팅에서 이 시그널이 결정에 영향이 있는지 청취.\n2. FCC·Texas 정부 관계자와의 업데이트 주기 강화.`,
    "samsung-next":     `1. 이 트렌드와 관련된 스타트업 후보를 4주 내 발굴 — 우리 사업부와 PoC 가능 여부 검토.\n2. 경쟁 VC의 투자 동향 모니터링.`,
    "strategy":         `1. 2-3년 후 시장 구조 시나리오에 이 트렌드 반영 — 회장 보고용 브리핑에 추가.\n2. 경쟁사(Apple/Google/TSMC) 대응 동향을 매주 점검.`,
  };
