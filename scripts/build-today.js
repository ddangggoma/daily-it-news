...
function buildConclusion(news) {
  ...
  // headline=true 가 score.js에서 IT gate(점수 4.0+ AND relevance 0.6+) 통과 시 set.
  // explicit headline이 있으면 그대로, 없으면 IT relevance 가장 높고 점수 좋은 항목 1차 후보.
  const explicit = news.find((n) => n.headline);
  // explicit이 없을 때는 IT 항목 중 점수 1위로. (비-IT는 demote)
  ...
  // 5초 결론의 종합 점수 = 헤드라인 자체 점수 (사용자 기대 일치)
  const scoreAvg = round2(topScore);
  // 임계 미달 시 약한 시그널 표시
  const headline = explicit || topScore >= 3.5
    ? top.title
    : `오늘 IT 주요 이슈가 약합니다 — 최고 점수 ${topScore.toFixed(1)}/5.0 (${top.title.slice(0, 60)}…)`;
...
function main() {
  ...
  for (const n of news) avgScore(n); // _avg 캐싱
